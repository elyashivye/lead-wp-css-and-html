...
class Plugin {
    ...
    public static function enqueue_admin_assets(string $hook): void {
        if (false === strpos($hook, 'lfb-leads') && false === strpos($hook, 'lfb_form')) {
            return;
        }
        wp_enqueue_style('lfb-admin', LFB_URL . 'assets/css/admin.css', [], '1.0.0');
    }
...
'
    ...
    public static function render_leads_page(): void {
        ...
        foreach ($leads as $lead) {
            ...
            foreach ($payload as $k => $v) {
                if (in_array((string) $k, ['wp_http_referer', '_wp_http_referer'], true)) {
                    continue;
                }
                if (!in_array((string) $k, $field_keys, true)) {
                    $field_keys[] = (string) $k;
                }
            }
        }

        echo '<div class="wrap lfb-admin-wrap"><h1>' . esc_html__('Leads', 'lead-forms-builder') . '</h1>';
        echo '<form method="get" class="lfb-toolbar">';
        ...
        echo '<table class="widefat striped lfb-leads-table"><thead><tr><th>ID</th><th>Form</th><th>Submitted At</th>';
        foreach ($field_keys as $field_key) {
            echo '<th>' . esc_html(ucwords(str_replace('_', ' ', $field_key))) . '</th>';
        }
        echo '</tr></thead><tbody>';
        if (!$leads) {
            echo '<tr><td colspan="' . esc_attr((string) (3 + count($field_keys))) . '">' . esc_html__('No leads found.', 'lead-forms-builder') . '</td></tr>';
        } else {
            foreach ($leads as $lead) {
                $form_id = (int) get_post_meta($lead->ID, '_lfb_form_id', true);
                $payload = (array) get_post_meta($lead->ID, '_lfb_payload', true);
                $created_at = (string) get_post_meta($lead->ID, '_lfb_created_at', true);
                echo '<tr><td>' . (int) $lead->ID . '</td><td>#' . $form_id . '</td><td>' . esc_html($created_at) . '</td>';
                foreach ($field_keys as $field_key) {
